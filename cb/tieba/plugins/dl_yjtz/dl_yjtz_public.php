...
if(isset($_GET['username'])){
        ...
        <tbody>
			<?php
				...
				for($ii=$min;$ii<=$max;$ii++){
					...
					if(($tieba['lastdo'] == date("Y-m-d",strtotime("-1 day"))) || (empty($tieba['lastdo']))){
						$tj = $tj + 1;
	...
	<thead>
        ...
        <tbody>
			<?php	
				...
				for($ii=$min;$ii<=$max;$ii++){
					$tieba = $m->fetch_array($m->query('select * from `'.DB_NAME.'`.`'.DB_PREFIX.$userinfo['t'].'` where `uid` = '.$userinfo['id'].' and `id`='.$ii));
					if((!empty($tieba['status'])) && ($tieba['lastdo'] == date('Y-m-d')) && (empty($tieba['no']))){
						$tj = $tj+1;
	...
	<thead>
        ...
        <tbody>
			<?php
				...
				for($ii=$min;$ii<=$max;$ii++){
					$tieba = $m->fetch_array($m->query('select * from `'.DB_NAME.'`.`'.DB_PREFIX.$userinfo['t'].'` where `uid` = '.$userinfo['id'].' and `id`='.$ii));
					if(($tieba['lastdo'] == date("Y-m-d")) && (empty($tieba['status'])) && (empty($tieba['no']))){
						$tj = $tj + 1;
    ...
    <br/><br/>插件作者：<a href="http://www.tbsign.cn" target="_blank">D丶L</a> // 程序作者：<a href="http://zhizhe8.net" target="_blank">无名智者</a> & <a href="http://longtings.com" target="_blank">Mokeyjay</a><br/><br/>
	<?php
	} else {
		die('警告：您无权访问此页面！');
		}
