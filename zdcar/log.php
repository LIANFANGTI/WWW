...
<body style="height:100%; background:url(img/login_bj.jpg) top center no-repeat" >
<?php
				...
				case "login":                                                         //登录功能
					...
					$cha=0;
						while($row=mysql_fetch_array($sql)){
							...
							$company[]=$row["company"];
							$id[]=$row["id"];
							if($user==$row["user"]&&md5($pwd)==$row["password"]){$cha=1;break;}
						}
						if($cha){
							session_start();
							...
							if (!check_j("user",$user))echo"用户名不存在";else echo"用户名或密码错误";
						}
				break;
				case "update":                                                           //数据更新					
					$name=$_GET["name"];
